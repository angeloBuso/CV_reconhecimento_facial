# Módulo para imprimir os componentes do PCA
...
class ResultsMontage:
	...
	def addResult(self, image, text=None, highlight=False):
		# Verifica se o número de imagens por linha foi obtido e, em caso afirmativo, redefine o contador de colunas e incrementa a linha
		if self.counter != 0 and self.counter % self.imagesPerRow == 0:
			self.col = 0
			self.row += 1

		# Redimensiona a imagem para a largura e a altura fixas e configura na montagem
		...
		# Se o texto não for None, desenhe
		if text is not None:
